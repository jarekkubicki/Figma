 // nazywanie sekcji preview BANNERY
...
if (selection.length === 0) {
  ...
  selection.forEach(node => {
    if (isValidNodeType(node)) {
      ...
      // Pobieranie sekcji dwa piętra wyżej - typ bannera
      let typeLevelsUpName = parentSections.length >= 2 ? parentSections[parentSections.length - 2] : 'Brak_sekcji';

      // Pobieranie sekcji GETIN
      let getinLevelsUpName = parentSections.length >= 3 ? parentSections[parentSections.length - 3] : 'Brak_sekcji';

      // Tworzenie nowej nazwy całości
      const newName = `${getinLevelsUpName}_${typeLevelsUpName}_preview`;
      node.name = newName;
    }
  });
